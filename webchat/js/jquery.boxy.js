...
function Boxy(element, options) {
    ...
    this.boxy.attr('boxId','box-'+Math.floor(Math.random()*100000000));
...
jQuery.extend(Boxy, {
    ...
    remove:function(s){
      if(typeof s=='string'){
        $('.boxy-wrapper[boxId="'+s+'"]').remove();
        $('.boxy-modal-blackout[boxId="'+s+'"]').remove();
        return;
      }else if(typeof s!='undefined'){
        var obj=$(s).parents('.boxy-wrapper');
        var boxId=obj.attr('boxId');
        obj.remove();
        $('.boxy-modal-blackout[boxId="'+boxId+'"]').remove();
        return;
      }
...
Boxy.prototype = {
    ...
    show: function() {
        ...
        if (this.options.modal) {
            ...
            this.modalBlackout = jQuery('<div class="boxy-modal-blackout" boxId="'+this.boxy.attr('boxId')+'"></div>')
                .css({zIndex: Boxy._nextZ(),
                      opacity: 0.5,
                      width: $(window).scrollLeft()+pageWidth(),
                      height: $(window).scrollTop()+pageHeight()})
    ...
    _setupTitleBar: function() {
        if (this.options.title) {
            ...
            if (this.options.closeable) {
                tb.append(jQuery("<a href='#' class='close' onclick='Boxy.remove()'></a>").html(this.options.closeText));
            }
    ...
    _setupDefaultBehaviours: function(root) {
        ...
        jQuery('.close', root).click(function() {
            self.hide();
            if(self.options.closeFunc!=null){
              self.options.closeFunc(self);
            }
            return false;
